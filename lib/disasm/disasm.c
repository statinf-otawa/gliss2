 ...
 */
void add_to_list(list_entry_t **m, const char *n, gliss_address_t a, uint32_t s) {
 ...
 * @return	0 if no label exists for the given address, non zero otherwise
*/
uint32_t get_size_from_list(list_entry_t *m, gliss_address_t addr) {
		...
		while (adr_start < adr_end) {
			...
			if(ee) {
				if(strncmp(ee->name, "$t", 2) == 0) {
					...
					gliss_address_t stop_data_at = adr_start;
					...
					while(adr_start < stop_data_at) {
						printf(" %8x:\t", adr_start);
						printf(".word %8x\n", gliss_mem_read32(gliss_get_memory(pf, 0), adr_start));
						adr_start += 4;
					}
